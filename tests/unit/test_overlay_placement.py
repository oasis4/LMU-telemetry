...
from lmu_telemetry.live.screens import Monitor

#: A 32:9 ultrawide, and an ordinary 16:9 one. Both at the origin, so these
#: say the same things they always did; the second-screen case, where the
#: origin is what matters, lives in test_screens.py.
ULTRAWIDE = Monitor(0, 0, 0, 5120, 1440, True)
NORMAL = Monitor(0, 0, 0, 1920, 1080, True)


def _place(position, screen, width=500, height=200, margin=28):
    return Overlay._place(position, screen, width, height, margin)
...
def test_every_position_keeps_the_whole_panel_on_the_screen():
    for position in POSITIONS:
        for screen in (ULTRAWIDE, NORMAL):
            ...
            assert x + 500 <= screen.width, (position, screen, x)
            assert 0 <= y, (position, screen, y)
            assert y + 200 <= screen.height, (position, screen, y)
...
def test_top_and_bottom_go_to_opposite_ends():
    ...
    assert top < bottom
    assert bottom + 200 <= NORMAL.height
